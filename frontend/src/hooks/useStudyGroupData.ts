...
export function useStudyGroupData(): StudyGroupData {
  ...
  const scrambleGroups = () => {
    const allMembers: string[] = groups.reduce(
      (acc: string[], group) => acc.concat(group.members),
      [],
    )
    const randomized = scrambleArr(allMembers)

    const numGroupsToSplit = groups.reduce(
      (acc, group) => (group.members.length > 0 ? acc + 1 : acc),
      0,
    )
